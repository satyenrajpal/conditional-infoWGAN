...
class Solver(object):
    ...
    def __init__(self, rafd_loader,mnist_loader, config):
    ...
    def train(self):
        ...
        for i in range(start_iters, self.num_iters):
            ...
            if (i+1) % self.sample_step == 0:
                with torch.no_grad():
                    for nC, c_start in enumerate(range(0,len(c_fixed_list),self.c_dim)):
                        x_fake_list=[]
                        
                        for c_fixed in c_fixed_list[c_start:c_start+self.c_dim]:
                            x_fake_list.append(self.G(c_fixed))
                        
                        x_concat = torch.cat(x_fake_list, dim=3)
                        sample_path = os.path.join(self.sample_dir, '{}-{}-images.jpg'.format(i+1,nC))
